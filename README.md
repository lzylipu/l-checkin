...
这个项目用于自动登录 [L站](https://站点域名/) 网站并随机读取几个帖子。它使用 Python 和 Playwright
...
| 环境变量名称             | 描述                                         | 示例值                          |
|--------------------|--------------------------------------------|------------------------------|
| `L_SITE_COOKIES`  | 从浏览器 DevTools 复制的 Cookie 字符串，设置后优先使用，无需账号密码 | `_t=xxx; _forum_session=yyy` |

> 获取方式：打开 [站点域名](https://站点域名/) 并登录 → 按 F12 → Application → Cookies → `https://站点域名` → 全选所有 Cookie 复制为字符串粘贴即可。

**方式二：账号密码登录**

| 环境变量名称             | 描述                | 示例值                                |
|--------------------|-------------------|------------------------------------|
| `L_SITE_USERNAME` | 你的 L站 用户名或邮箱 | `your_username` 或 `your@email.com` |
| `L_SITE_PASSWORD` | 你的 L站 密码     | `your_password`                    |

> 若同时设置了 `L_SITE_COOKIES` 和账号密码，**Cookie 登录优先**；Cookie 失效时自动回退到账号密码登录。
...
1. **设置环境变量**：
    - 在 GitHub 仓库的 `Settings` -> `Secrets and variables` -> `Actions` 中添加以下变量：
        - （二选一）`L_SITE_COOKIES`：从浏览器复制的 Cookie 字符串（**推荐，优先使用**）。
        - （二选一）`L_SITE_USERNAME` + `L_SITE_PASSWORD`：你的 L站 用户名/邮箱和密码。
...
2. **添加仓库**
    ...
    - 依次在对应的字段填入内容（未提及的不填）：
      - **名称**：L站 签到
      - **类型**：公开仓库
      - **链接**：https://github.com/lzylipu/l-checkin.git
      - **分支**：main
      - **定时类型**：`crontab`
...
3. **配置环境变量**
    ...
    - 需要配置以下变量：
        - （二选一）`L_SITE_COOKIES`：从浏览器复制的 Cookie 字符串（**推荐，优先使用**）
        - （二选一）`L_SITE_USERNAME`：你的L站用户名/邮箱
        - （二选一）`L_SITE_PASSWORD`：你的L站密码
